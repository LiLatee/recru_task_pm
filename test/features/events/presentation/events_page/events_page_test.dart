...
class MockEventsPageVerticalListCubit extends Mock implements EventsPageVerticalListCubit {}

class MockEventsPageHorizontalListCubit extends Mock implements EventsPageHorizontalListCubit {}

void main() {
  late EventsPageVerticalListCubit mockEventsPageVerticalListCubit;
  late EventsPageHorizontalListCubit mockEventsPageHorizontalListCubit;

  setUp(() {
    mockEventsPageVerticalListCubit = MockEventsPageVerticalListCubit();
    final verticalDataState = EventsPageVerticalListCubitDataState(items: [
    ...
    when(() => mockEventsPageVerticalListCubit.state).thenAnswer((_) => verticalDataState);
    when(() => mockEventsPageVerticalListCubit.stream).thenAnswer((_) => Stream.fromIterable([verticalDataState]));
    when(() => mockEventsPageVerticalListCubit.close()).thenAnswer((_) async => () {});

    mockEventsPageHorizontalListCubit = MockEventsPageHorizontalListCubit();
    final horizontalDataState = EventsPageHorizontalListCubitDataState(items: const [
    ...
    when(() => mockEventsPageHorizontalListCubit.state).thenAnswer((_) => horizontalDataState);
    when(() => mockEventsPageHorizontalListCubit.stream).thenAnswer((_) => Stream.fromIterable([horizontalDataState]));
    when(() => mockEventsPageHorizontalListCubit.close()).thenAnswer((_) async => () {});
  });

  goldenTest(
    name: '$EventsPage - data',
    builder: (_) => EventsPage(
      eventsPageVerticalListCubit: mockEventsPageVerticalListCubit,
      eventsPageHorizontalListCubit: mockEventsPageHorizontalListCubit,
    ),
  );

  goldenTest(
    name: '$EventsPage - scroll to bottom',
    action: (tester) async {
      await tester.scrollToBottom();
    },
    builder: (_) => EventsPage(
      eventsPageVerticalListCubit: mockEventsPageVerticalListCubit,
      eventsPageHorizontalListCubit: mockEventsPageHorizontalListCubit,
  ...
  goldenTest(
    name: '$EventsPage - pull to refresh',
    action: (tester) async {
  ...
  goldenTest(
    name: '$EventsPage - loading',
    setup: (_) async {
      final verticalLoadingState = EventsPageVerticalListCubitLoadingState();
      when(() => mockEventsPageVerticalListCubit.state).thenAnswer((_) => verticalLoadingState);
      when(() => mockEventsPageVerticalListCubit.stream).thenAnswer((_) => Stream.fromIterable([verticalLoadingState]));

      final horizontalLoadingState = EventsPageHorizontalListCubitLoadingState();
      when(() => mockEventsPageHorizontalListCubit.state).thenAnswer((_) => horizontalLoadingState);
      when(() => mockEventsPageHorizontalListCubit.stream)
          .thenAnswer((_) => Stream.fromIterable([horizontalLoadingState]));
    },
    builder: (_) => EventsPage(
      eventsPageVerticalListCubit: mockEventsPageVerticalListCubit,
      eventsPageHorizontalListCubit: mockEventsPageHorizontalListCubit,
  ...
  goldenTest(
    name: '$EventsPage - error',
    setup: (_) async {
      final verticalErrorState = EventsPageVerticalListCubitErrorState(failure: const FailureWithMessage('message'));
      when(() => mockEventsPageVerticalListCubit.state).thenAnswer((_) => verticalErrorState);
      when(() => mockEventsPageVerticalListCubit.stream).thenAnswer((_) => Stream.fromIterable([verticalErrorState]));

      final horizontalErrorState =
          EventsPageHorizontalListCubitErrorState(failure: const FailureWithMessage('message'));
      when(() => mockEventsPageHorizontalListCubit.state).thenAnswer((_) => horizontalErrorState);
      when(() => mockEventsPageHorizontalListCubit.stream)
          .thenAnswer((_) => Stream.fromIterable([horizontalErrorState]));
    },
    builder: (_) => EventsPage(
      eventsPageVerticalListCubit: mockEventsPageVerticalListCubit,
      eventsPageHorizontalListCubit: mockEventsPageHorizontalListCubit,
